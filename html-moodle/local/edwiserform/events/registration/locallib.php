...
class edwiserform_events_registration extends edwiserform_events_plugin {
    ...
    /**
     * Confirm user account. Current user can confirm his account only if he has submission
     * in passed formid. Also username and usersecret key need to matched with database record.
     * @param  integer $formid     Form id
     * @param  string  $username   Username
     * @param  string  $usersecret Secret key to confirm user account
     * @return integer             Confirmation status
     */
    public function confirm_user($formid, $username, $usersecret) {
        global $DB;
        ...
        $sql = 'SELECT f.events
                 ...
                 WHERE f.id = ?
                   AND fd.userid = ?
                   AND f.events LIKE "%registration%"';
        if (!$DB->get_record_sql($sql, array($formid, $user->id))) {
            return INVALID_DATA;
        }
        $user->confirmed = true;
        $DB->update_record('user', $user);
    ...
    public function visible_url($form) {
        global $CFG;
        return $CFG->wwwroot . '/login/signup.php';
    }
}
