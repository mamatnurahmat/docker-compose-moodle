...
class edwiserform {
     ...
     * @param string $data user submission
    ...
    public function cron() {
        ...
        mtrace(get_string('efb-cron-start', 'local_edwiserform'));
        foreach ($forms as $form) {
            ...
            if ($data) {
                mtrace(get_string('efb-delete-form-user-files-cron-start', 'local_edwiserform', $form->id));
                foreach ($data as $userdata) {
                    // If there is files in the submission then delete it first.
                    if (stripos($userdata->submission, '"type":"file"') != -1) {
                        $this->delete_user_uploaded_files($userdata);
                    }
                    // If form has any events enabled then trigger its submission_deletion_action.
                    if (!empty($events)) {
                        foreach ($events as $plugin) {
                            mtrace($plugin->submission_deletion_action($form, $userdata));
                        }
                    }
                }
                mtrace(get_string('efb-delete-form-user-files-cron-end', 'local_edwiserform', $form->id));
            }
            try {
            ...
            if (!empty($events)) {
                foreach ($events as $plugin) {
                    mtrace($plugin->form_deletion_action($form, $userdata));
                }
            }
        }
        $fs = get_file_storage();
        $fs->delete_area_files(context_system::instance()->id, EDWISERFORM_COMPONENT, 'export');
        mtrace(get_string('efb-cron-end', 'local_edwiserform'));
    }
    ...
    public function view($page) {
        ...
        switch ($page) {
            case 'newform':
                ...
                $themedependentcss = '/local/edwiserform/style/formedit_' . $CFG->theme . '.css';
                if (file_exists($CFG->dirroot . $themedependentcss)) {
                    $css[] = new moodle_url($CFG->wwwroot . $themedependentcss);
                }
